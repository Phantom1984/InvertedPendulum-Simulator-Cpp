...
这个代码的模型对应小车摆杆逆时针为正,向右为正的模型(标准模型)
*/
#include <iostream>
#include <math.h>
#include <fstream>
...
int main() {
    ...
    // 初始条件：偏离平衡位置20度
    pendulum.setState(0.0, 0.0, 20*M_PI/180.0, 0);
    ...
    while (time < 50.0) {  // 模拟50秒
        ...
        //角度限幅,限制在[-180, 180]
        double theta_limit = pendulum.normalizeAngle(state[2]);

        // 角度控制:内环
        double control_pid = angle_pid.update(0.0*M_PI/180.0, theta_limit, dt);  //将外环计算的角度控制量传入内环

        // 更新系统状态
        pendulum.update(control_pid, dt);
